...
- [ ] **Voice "lex end session" = End button parity (Fix 30).** Currently divergent: voice end_session runs the brainstorm-end pipeline only (`07-daemon/src/voice/lex-voice-ws.ts:1611-1618`), while clicking End in `LexSessionList.tsx:370` calls `endLexAnchor(id)` which kills the PTY/anchor. User expectation is that voice command is literally equivalent to button click. Fix: after `fireSessionEndPipeline('voice-command')` returns, also call the anchor-end path so the row flips dormant and the End button hides. Direct-llm anchors have no PTY so the cost is just the dormant flip; cc-pty anchors lose the worker PTY by intent (clean teardown matches the spoken "end session"). Ship as Fix 30, dashboard tile flip is already covered by the existing `brainstorm-ended` SSE + lex-anchors refetch. Risk: low for direct-llm (current standalone Lex usage), higher for cc-pty anchors if a worker is mid-task; gate behind a check that no in-flight tool_use exists on the worker before tearing the anchor.
- [ ] **Utterance queue coalesce (relevance-aware single structured reply).** Spec memory: `project_devneural_utterance_queue_coalesce.md`. Smoke confirmed double-talk persists; fix not shipped. Ship FIRST after smoke, ahead of autonomy stages.
- [ ] **Stages 5-12 of `docs/spec/LEX-AUTONOMY-PAYLOAD-SPEC.md`** (codex-reordered). Gated behind live smoke of Stage 0-2 + repoint fix + voice mic-init fix + coalesce ship. Codex warning baked in spec line 295: do not ship 5-9 before 1-4 verified. Remaining: distillation query scope fix, sync barrier + freshness metadata, stale/failure surfacing, adaptive walk-back, worker boot payload builder, first-attach path, loose-ends handoff gate, grooming/escalation, project_scope_id.
